...
The reflex for "stacking toasts" in Vue is a `ref<Banner[]>` — push on
trigger, filter on dismiss, map to render. This example keeps no such
array. Every `ErrorBannerCall.call(…)` is an independent entry in the
