...
`createCallable` is an **unmounting delay** in milliseconds: call-vue
...
- **Out** — `call.end(...)` sets `call.ended`, `open` goes `false`, the
  panel slides back out, and call-vue unmounts it once the delay elapses.
...
- `call.ended` is per-call — a click-outside, a button, or an external
  `SettingsDrawerCall.end(null)` all route through the same window, so
  every close animates the same way.
